...
// import { getProducts, type Products } from "@/services/product.service";
...
export const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      setSearchQuery(searchInput); // update query after user stops typing
  ...
  const mappedProducts = paginatedProducts.map((p) => ({
    id: p.id,
    name: p.name,
    category: p.category?.name || "Uncategorized", // backend category object -> string
    price: p.price,                         // backend price string -> number
    qty: p.qty,
    image: p.images?.[0]?.imageUrl || "",           // use first image or empty string
  }));
          ...
          <div className="max-w-350 mx-auto px-8 lg:px-12 py-16 md:py-24">
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
              <div className="space-y-8 lg:pr-8">
                ...
                <p className="text-xl text-muted-foreground font-light tracking-tight max-w-xl">
        ...
        <div className="max-w-350 mx-auto px-8 lg:px-12 py-16 md:py-20">
          <div className="flex items-center justify-center space-x-3 flex-wrap gap-3">
            {categories.map((category) => (
              <button
                key={category}
                className={`px-8 py-3.5 rounded-full font-medium text-sm transition-all ${selectedCategory === category
                  ? "bg-black text-white"
  ...
  <div className="flex w-full max-w-xl gap-2">
    <div className="relative flex-1">
      <form
        onSubmit={(e) => e.preventDefault()} // prevent Enter from refreshing
        className="relative"
      >
        <input
          type="text"
        ...
        {/* Products Grid */}
        <div className="max-w-350 mx-auto px-8 lg:px-12 pb-24 md:pb-32">
